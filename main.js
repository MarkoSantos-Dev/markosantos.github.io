...
// Torus
const geometry = new THREE.TorusGeometry(10, 3, 16, 100);
const material = new THREE.MeshStandardMaterial({ color: 0x892500 });
const torus = new THREE.Mesh(geometry, material);
scene.add(torus);
...
// Avatar (your cube with picture)
const markoTexture = new THREE.TextureLoader().load('/Images/Marko3.png');
const marko = new THREE.Mesh(
  new THREE.BoxGeometry(3, 3, 3),
  new THREE.MeshBasicMaterial({ map: markoTexture })
);
scene.add(marko);
...
function moveCamera() {
  ...
  if (t === 0) {
    // Smoothly reset rotation AND position when back at top
    marko.rotation.x += (0 - marko.rotation.x) * 0.1;
    marko.rotation.y += (0 - marko.rotation.y) * 0.1;
    marko.rotation.z += (0 - marko.rotation.z) * 0.1;
